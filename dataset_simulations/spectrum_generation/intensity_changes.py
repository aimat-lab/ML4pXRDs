...
class TextureGen(object):
    ...
    def pattern(self):
        struc = self.struc
        return self.calculator.get_pattern(
    ...
    def textured_intensities(self):
        ...
        if self.struc.lattice.is_hexagonal() == True:
            ...
            while check == 0.0:
                ...
                check = np.dot(
                    np.array(preferred_direction), np.array(preferred_direction)
                )  # Ensure 0-vector is not used
        ...
        for (hkl, peak) in zip(hkls, intensities):
            ...
            norm_2 = math.sqrt(
                np.dot(np.array(preferred_direction), np.array(preferred_direction))
            )
            total_norm = norm_1 * norm_2
            texture_factor = abs(
                np.dot(np.array(hkl), np.array(preferred_direction)) / total_norm
            )
...
def main(struc, num_textured, max_texture=0.6, min_angle=10.0, max_angle=80.0):
    ...
    textured_patterns = [
        texture_generator.textured_spectrum for i in range(num_textured)
    ]
