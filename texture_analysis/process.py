...
# use this as a test diffractogram
...
def fit_baseline(xs, ys):
    ...
    ys_baseline = baseline_arPLS(ys, 0.1, lam=10 ** 8)
...
ys_smoothed = (cumsum_vec[window_width:] - cumsum_vec[:-window_width]) / window_width

# TODO: Don't do this
cumsum_vec = np.cumsum(np.insert(xs_test, 0, 0))
xs_smoothed = (cumsum_vec[window_width:] - cumsum_vec[:-window_width]) / window_width

peaks, props = find_peaks(
    ys_smoothed, distance=0.5 / (xs_smoothed[1] - xs_smoothed[0]), prominence=40
)
