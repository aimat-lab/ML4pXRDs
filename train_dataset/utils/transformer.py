...
class TransformerPositionalEmbedding(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        x = keras.layers.TimeDistributed(self.embedding)(
            inputs
        )  # embed 1D sequence into self.width-D sequence (time-independently)
...
def build_model_transformer(
    ...
    transformer_model = ModelTrunk(
        (None, number_of_input_values, 1),
        num_heads=8,
        head_size=16,
        num_layers=1,
        input_embedding_width=2,
    )  # TODO: Switch back to 128 head_size
...
def get_lr_scheduler_warmup_callback(
    warmup_epochs=15, decay_epochs=100, initial_lr=1e-6, base_lr=1e-3, min_lr=5e-5
):
