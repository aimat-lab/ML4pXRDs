...
class UNet:
    ...
    def UNet(self):
        ...
        for j in range(0, self.model_depth):
            if self.D_S == 1:
                ...
                level = tf.keras.layers.Conv1D(
                    1, 1, name=f"level{self.model_depth - j}"
                )(deconv)
                levels.append(level)
            if self.is_transconv:
                deconv = Concat_Block(
                    trans_conv1D(
                        deconv, self.model_width, 2 ** (self.model_depth - j - 1)
                    ),
                    convs_list[self.model_depth - j - 1],
                )
            elif not self.is_transconv:
                deconv = Concat_Block(
                    upConv_Block(deconv), convs_list[self.model_depth - j - 1]
                )
            deconv = Conv_Block(
                deconv,
                self.model_width,
                self.kernel_size,
                2 ** (self.model_depth - j - 1),
            )
            deconv = Conv_Block(
                deconv,
                self.model_width,
        ...
        if self.problem_type == "Classification":
            outputs = tf.keras.layers.Conv1D(
                self.output_nums, 1, activation="softmax", name="out"
            )(deconv)
        elif self.problem_type == "Regression":
            outputs = tf.keras.layers.Conv1D(
                self.output_nums, 1, activation="linear", name="out"
            )(deconv)
    ...
    def UNetE(self):
        ...
        convs_list = list(convs.values())
        if self.D_S == 1:
            level = tf.keras.layers.Conv1D(1, 1, name=f"level{self.model_depth}")(
                convs_list[0]
            )
            levels.append(level)
        ...
        for i in range(1, (self.model_depth + 1)):
            for j in range(0, (self.model_depth - i + 1)):
                ...
                if (self.D_S == 1) and (j == 0) and (i < self.model_depth):
                    level = tf.keras.layers.Conv1D(
                        1, 1, name=f"level{self.model_depth - i}"
    ...
    def MultiResUNet(self):
        ...
        for i in range(1, (self.model_depth + 1)):
            mresblock = MultiResBlock(
                pool, self.model_width, self.kernel_size, 2 ** (i - 1), self.alpha
            )
            pool = tf.keras.layers.MaxPooling1D(pool_size=2)(mresblock)
            mresblocks["mres%s" % i] = ResPath(
                mresblock,
                (self.model_depth - i + 1),
                self.model_width,
                self.kernel_size,
                2 ** (i - 1),
            )
        ...
        for j in range(0, self.model_depth):
            ...
            if self.is_transconv:
                deconv = Concat_Block(
                    ...
                    mresblocks_list[self.model_depth - j - 1],
                )
            elif not self.is_transconv:
                deconv = Concat_Block(
                    upConv_Block(deconv), mresblocks_list[self.model_depth - j - 1]
                )
            deconv = MultiResBlock(
                deconv,
                self.model_width,
                self.kernel_size,
                2 ** (self.model_depth - j - 1),
